...
class ProductCategoryVendorDiscount(models.Model):
    _name = "product.category.vendordiscount"
...
class ProductCategory(models.Model):
    ...
    mrx_seller_ids = fields.One2many('product.category.vendordiscount', 'name', string='Vendors', help="Define vendor discount.")

# Adds one2many field to the partner table
class Partner(models.Model):
    _inherit = "res.partner"
    
    mrx_category_ids = fields.One2many('product.category.vendordiscount', 'partner_id', string='Product Category', help="Define vendor discount.")
